...
/**
 * Class UpgradeSchema
 * @package Magetop\Productslider\Setup
 */
class UpgradeSchema implements UpgradeSchemaInterface
{
    ...
    public function upgrade(SchemaSetupInterface $setup, ModuleContextInterface $context)
    {
        ...
        if (version_compare($context->getVersion(), '2.0.0', '<')) {
            if ($setup->tableExists('magetop_productslider_slider')) {
                $connection->dropTable($setup->getTable('magetop_productslider_slider'));
            }
            $table = $connection->newTable($setup->getTable('magetop_productslider_slider'))
