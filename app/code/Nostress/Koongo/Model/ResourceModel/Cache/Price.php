...
class Price extends \Nostress\Koongo\Model\ResourceModel\Cache\Product
{
	...
	*/
    ...
    protected function getConfigurablePriceSql()
    {
    	...
    	$select = $this->getEmptySelect();
    	$select->from(array($this->_mainTableAlias => $this->getMainTable()), 
    			array( "product_id" => $cprTableAlias.".parent_id",
	...
	{							
		...
		$select->from(array($tierPriceCacheTableAlias =>  $tierPriceCacheTable), ["product_id","store_id","customer_group_id"]);
