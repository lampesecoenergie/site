...
{   
	...
	protected $_commonColumns = array("product_id","website_id",self::WEEE_COLUMN_TOTAL);
    ...
    protected function getSelectWeeeSql()
    {
	    ...
	    $select->from(array($mainTableAlias => $mainTable), array('product_id' => 'entity_id','website_id' => "({$this->getWebsiteId()})",self::WEEE_COLUMN_TOTAL => "(0.00)"));
    ...
    protected function joinWeeeAttribute($select,$attributeCode,$attributeId)
    {
    	...
    	$select->columns(array($attributeCode => $value));
