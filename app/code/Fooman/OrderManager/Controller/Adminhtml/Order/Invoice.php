...
use Fooman\OrderManager\Model\InvoiceProcessor;
    ...
    public function execute()
    {
        ...
        foreach ($collection->getItems() as $order) {
            try {
                ...
                $this->invoiceProcessor->invoice($order->getEntityId(), $sendEmails);
                $this->invoicedOrderIds[] = $order->getEntityId();
                $countSuccess++;
            } catch (\Exception $e) {
        ...
        if ($countFailure && $countSuccess) {
            $this->messageManager->addErrorMessage(__('%1 order(s) were not invoiced.', $countFailure));
        } elseif ($countFailure) {
            $this->messageManager->addErrorMessage(__('No order(s) were invoiced.'));
        }

        if ($countSuccess) {
            $this->messageManager->addSuccessMessage(__('%1 order(s) were invoiced.', $countSuccess));
        }
