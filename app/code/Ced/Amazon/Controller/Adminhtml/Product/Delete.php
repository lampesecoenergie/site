...
class Delete extends \Magento\Backend\App\Action
{
     ...
     * @param \Ced\Amazon\Helper\Product\Product $product
    ...
    public function execute()
    {
        ...
        if (isset($filters)) {
            ...
            $message = 'Product(s) delete request processed successfully.';
            if ($throttle) {
            ...
            $response = $this->product->update($ids, $throttle, 'Delete');
        }
        ...
        if ($response) {
            ...
            $this->messageManager->addComplexErrorMessage(
                'addAmazonError',
                [
                    'message' => 'Feed product delete upload failed.',
                    'reasons' => [
                        'Profile must be disabled.',
                        'Account must be disabled.',
                        'Product must be invalid.',
                        'Product is not available or uploaded on the marketplace.'
                    ],
                    'support_url' => $this->getUrl('integrator/support/index')
                ]
            );
