...
{
    const CLI_NAME = self::CLI_PREFIX . 'amazon:queue:flush';

    protected function configure()
    {
        $this->setName(self::CLI_NAME);
        $this->setDescription('Clear queues via cli');
        $this->addArgument('all', \Symfony\Component\Console\Input\InputArgument::OPTIONAL, 'delete all queues');
        $this->addOption(
            'log',
            'd',
    ...
    protected function execute(
        ...
        $progress->advance();
        usleep(300000);
        /** @var \Ced\Amazon\Cron\Queue\Cleaner $processor */
        $processor = $this->om->create(\Ced\Amazon\Cron\Queue\Cleaner::class);
        if (!empty($all)) {
            $processor->all = true;
        }
        ...
        if ($log) {
            ...
            $logger->info("Queue flush cron run by schedule.");
        }
    }
}
