...
class Price extends \Ced\Integrator\Console\Base
{
    const CLI_NAME = self::CLI_PREFIX . 'amazon:product:price';

    protected function configure()
    {
        $this->setName(self::CLI_NAME);
        $this->setDescription('Sync product price via cli');
        $this->addOption(
            'log',
            'd',
    ...
    protected function execute(
        ...
        $processor = $this->om->create(\Ced\Amazon\Cron\Product\Price::class);
        $processor->execute();
        $progress->advance();
        usleep(300000);
        $progress->finish();
        ...
        if ($log) {
            ...
            $logger->info("All price add cron run by schedule.");
        }
    }
}
