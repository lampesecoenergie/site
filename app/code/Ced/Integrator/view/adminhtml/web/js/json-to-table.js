define([], function () {
    ...
    return {
         ...
         *     var jsonHtmlTable = require("j2t").convert(eval(dataString), 'jsonTable', null, 'Download');
         *
         ...
         * @param theme object Optional table css class name
         * @param linkText string Optional text replacement for link pattern
         *
         * @return string Converted JSON to HTML table
         */
        convert: function (parsedJson, tableId, theme, linkText) {
            ...
            var classMarkup = theme.table ? ' class="' + theme.table + '"' : '';
            var thClass = theme.th ? ' class="' + theme.th + '"' : '';
            var tdClass = theme.td ? ' class="' + theme.td + '"' : '';
            var trClass = theme.tr ? ' class="' + theme.tr + '"' : '';
            ...
            var tr = '<tr ' + trClass + '>{0}</tr>';
            var thRow = '<th ' + thClass + '>{0}</th>';
            var tdRow = '<td ' + tdClass + '>{0}</td>';
            var thCon = '';
            var tbCon = '';
            var trCon = '';
            ...
            if (parsedJson) {
                ...
                if (isStringArray) {
                    ...
                    if (this.isType('array', parsedJson) && typeof (parsedJson[0]) == 'object') {
                        headers = this.array_keys(parsedJson[0]);

                        for (var i = 0; i < headers.length; i++) {
                            thCon += thRow.format(headers[i]);
                        }
                    } else if (this.isType('object', parsedJson)) {
                ...
                if (isStringArray) {
                    ...
                    if (headers) {
                        ...
                        if (this.isType('array', parsedJson)) {
                            for (var i = 0; i < parsedJson.length; i++) {
                                    ...
                                    if (isUrl) {
                                        ...
                                        if (value) {
                                            if (typeof (value) == 'object') {
                                                ...
                                                tbCon += tdRow.format(this.convert(eval(value), value.tableId, value.theme, value.linkText));
                                            } else {
                            ...
                            for (var index in parsedJson) {
                                if (parsedJson.hasOwnProperty(index)) {
                                    var value = parsedJson[index];
                                    var isUrl = urlRegExp.test(value) || javascriptRegExp.test(value);
        ...
        array_keys: function (input, search_value, argStrict) {
            var search = typeof search_value !== 'undefined', tmp_arr = [], strict = !!argStrict, include = true,
                key = '';
