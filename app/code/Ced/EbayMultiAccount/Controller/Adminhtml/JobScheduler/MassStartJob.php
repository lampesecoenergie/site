 ...
 */
class MassStartJob extends Action
{
    /**
     * @var PageFactory
     */
    ...
    public function execute()
    {
        ...
        if (!empty($feedIds)) {
            ...
            $this->_session->setFeedIdsForStartUpload($feedIds);
            $resultPage = $this->resultPageFactory->create();
            $resultPage->setActiveMenu('Ced_EbayMultiAccount::product');
            $resultPage->getConfig()->getTitle()->prepend(__('Start Upload Job'));
            return $resultPage;
        } else {
            $this->messageManager->addErrorMessage(__('No Record Found To Start Upload'));
