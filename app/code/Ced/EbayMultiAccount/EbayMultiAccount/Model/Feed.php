...
class Feed extends \Magento\AdminNotification\Model\Feed
{
    ...
    const XML_FEED_URL_PATH     = 'system/ebaymultiaccount/feed_url';
    const XML_FREQUENCY_PATH    = 'system/ebaymultiaccount/frequency';
    const XML_LAST_UPDATE_PATH  = 'system/ebaymultiaccount/last_update';
    ...
    public function checkUpdate()
    {
        ...
        $feedXml = $this->getFeedData($this->_objectManager->get('Ced\EbayMultiAccount\Helper\Feed')->getEnvironmentInformation());
       
        $allowedFeedType = explode(',', $this->_backendConfig->getValue(self::XML_FEED_TYPES));
        
        if ($feedXml && $feedXml->channel && $feedXml->channel->item) {
            $installedModules=$this->_objectManager->get('Ced\EbayMultiAccount\Helper\Feed')->getCedCommerceExtensions();
            foreach ($feedXml->channel->item as $item) {
               
                if(!isset($installedModules[(string)$item->module])) {
                    continue;
                }
                if($this->_objectManager->get('Ced\EbayMultiAccount\Helper\Feed')->isAllowedFeedType($item)) {
