 ...
 */
class ScheduleBulkInventory extends \Magento\Backend\App\Action
{
    /**
    ...
    public function execute()
    {
        ...
        if (count($prodCollection) > 0) {
            if(count($prodCollection) > 100) {
                $this->_session->setBulkInventoryIds($prodCollection);
                $resultPage = $this->resultPageFactory->create();
                $resultPage->setActiveMenu('Ced_EbayMultiAccount::product');
                $resultPage->getConfig()->getTitle()->prepend(__('Schedule Product Ids For Inventory/Price Syncing'));
                return $resultPage;
            }
            ...
            if (!$hasError) {
                $this->messageManager->addSuccessMessage(__('Product Ids Scheduled for Inventory/Price Sync.'));
            } else {
                $this->messageManager->addErrorMessage('Something Went Wrong while scheduling Product Ids Scheduled for Inventory/Price Sync');
            }
        } else {
