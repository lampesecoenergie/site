...
class ShipmentHelper extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    {
        $this->logger->addInfo('In Order Shipment Cron');
        $EbayMultiAccountOrderCollection = $this->objectManager->create('\Ced\EbayMultiAccount\Model\Orders')->getCollection()
            ->addFieldToFilter('status' , 'acknowledge')
            //->addFieldToFilter('ebaymultiaccount_order_id' , '323845777542-2105573810011')
            /*->setPageSize(10)
            ->setCurPage(1)*/
            ->setOrder('id', 'DESC')
            /*->getData()*/;
        ...
        foreach ($EbayMultiAccountOrderCollection as $EbayMultiAccountOrder) {
            $magentoOrderId = $EbayMultiAccountOrder->getMagentoOrderId();
            ...
            if($order->getStatus() == 'complete' || $order->getStatus() == 'Complete' ) {
                ...
                $this->logger->addInfo('In Order Shipemnt Cron: '.$return .' Order Id '. $magentoOrderId, ['path' => __METHOD__]);
            }
        }
        return true;
    }
    ...
    public function shipment($orders = null, $ebaymultiaccountOrder = null)
    {
        ...
        try{
            ...
            $shipStationcarrier = isset($trackArray['title']) ? $trackArray['title'] :
            ...
            $shippingCarrierUsed = isset($mappedShippingMethods[$shipStationcarrier]) ? $mappedShippingMethods[$shipStationcarrier] : $shipStationcarrier;
