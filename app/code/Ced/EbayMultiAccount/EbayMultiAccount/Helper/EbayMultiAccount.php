...
class EbayMultiAccount extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    {
        $VariationSpecificPictureSet = $modifySpecificsName = $valuelist ='';
        $productType = $product->getTypeInstance();
        $attrs = $product->getTypeInstance(true)->getConfigurableAttributesAsArray($product);
        foreach ($attrs as $attr) {
            if(!isset($imgVarLabel) || $imgVarLabel == '') {
                $imgVarLabel = $attr['label'];
            }
            if(!isset($imgVarCode) || $imgVarCode == '') {
                $imgVarCode = $attr['attribute_code'];
            }
            $values = '';
            $varSpecificsSet = '<NameValueList>
        ...
        foreach ($configProd as $childprod) {
            ...
            $qty = $this->getQuantityForUpload($childprod);
            $prepareVar = '<Variation>
                            <SKU>'.$childprod->getSku().'</SKU>
                            <StartPrice>'.$this->getEbayMultiAccountPrice($childprod).'</StartPrice>
            ...
            $allImg = $childprod->getMediaGalleryImages();
            $pictureUrls = '';
            $imageIndex = 1;

            $baseImage = $this->objectManager->get('Magento\Catalog\Model\Product\Media\Config')->getMediaUrl($childprod->getImage());
            if ($baseImage) {
                if ($handle = @fopen($baseImage, 'r')) {
                    $pictureUrls .= "<PictureURL>".$baseImage."</PictureURL>";
                    $imageIndex++;
                }
            }
            ...
            foreach ($allImg as $image) {
                ...
                $pictureUrls .= "<PictureURL>".$image->getUrl()."</PictureURL>";
                $imageIndex++;
            }
            $attrValue = isset($imgVarCode) ? $this->getMagentoProductAttributeValue($childprod, $imgVarCode) : '';
            if($attrValue && $pictureUrls) {
                $VariationSpecificPictureSet .= '<VariationSpecificPictureSet>' . $pictureUrls . '
                  <VariationSpecificValue><![CDATA[' . $attrValue . ']]></VariationSpecificValue>
                </VariationSpecificPictureSet>';
            }
        }

        if(isset($imgVarLabel) && $imgVarLabel && isset($VariationSpecificPictureSet) && $VariationSpecificPictureSet) {
            $picturesData = '<Pictures>
                <VariationSpecificName><![CDATA[' . $imgVarLabel . ']]></VariationSpecificName>'
                . $VariationSpecificPictureSet .
                '</Pictures>';
        }

        $finalXml = !empty($variations) ? '<Variations>'.$modifySpecificsName.$varSpecSetArray.$variations.$picturesData.'</Variations>' : '';
        return $finalXml;
    }
