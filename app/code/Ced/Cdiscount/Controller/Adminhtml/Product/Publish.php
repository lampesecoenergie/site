    ...
    public function execute()
    {
        ...
        if (!$this->cdiscount->checkForConfiguration()) {
            $this->messageManager->addErrorMessage(
                __('Products Publish Failed. Cdiscount API not enabled or Invalid. Please check Cdiscount Configuration.')
            );
        ...
        if (isset($batchId)) {
            ...
            $response = $this->cdiscount->publishProducts($productIds[$batchId]);
            if (isset($productIds[$batchId]) && $response) {
                return $resultJson->setData(
                    [
                        'success' => count($productIds[$batchId]) . " Product(s) Published successfully",
            ...
            return $resultJson->setData(
                [
                    'error' => count($productIds[$batchId]) . " Product(s) Published Failed",
        ...
        if (count($productIds) == 0) {
            $this->messageManager->addErrorMessage('No Product selected to publish.');
            return $redirect->setPath(\Ced\Cdiscount\Controller\Adminhtml\Product\Validate::REDIRECT_PATH);
        }

        // case 3.1 normal uploading if current ids are equal to chunk size.
        if (count($productIds) <= self::CHUNK_SIZE) {
            $response = $this->cdiscount->publishProducts($productIds);
            if ($response) {
                if (is_bool($response)) {
                    $this->messageManager->addSuccessMessage(count($productIds) . ' Product(s) Published Successfully');
                } else {
                    $this->messageManager->addSuccessMessage($response);
                }
            } else {
                $message = 'Product(s) Publish Failed.';
        ...
        $resultPage->getConfig()->getTitle()->prepend(__('Publish Products'));
        return $resultPage;
    }
}
