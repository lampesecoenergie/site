...
";//$file['html'];
    ...
    public function getFileLink($filePath)
    {
        ...
        $file = $this->dl->getPath('media')."/rueducommerce/" . $fileName;
        if ($this->file->fileExists($file)) {
            $url = $this->urlBuilder->getBaseUrl() . "pub/media/rueducommerce/" . $fileName;
            $html = "<a href='" . $url ."' download'>$fileName</a>";
        } else {
            if ($this->file->fileExists($filePath)) {
                $cpDir = $this->dl->getPath('media')."/rueducommerce/";
