...
namespace Ced\RueDuCommerce\Model\ResourceModel;
...
class ProfileProduct extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
    ...
    {
        $this->_init('rueducommerce_profile_products', 'id');
    }
    ...
    public function deleteFromProfile($productId)
    {
        ...
        $condition = "{$this->getTable('rueducommerce_profile_products')}.product_id = " . $dbh->quote($productId);
        //. " AND {$this->getTable('rueducommerce/profileproducts')}.profile_id = " . $dbh->quote($profileId);
        $dbh->delete($this->getTable('rueducommerce_profile_products'), $condition);
        return $this;
    }
    ...
    public function deleteProducts($productIds)
    {
        ...
        $dbh = $this->getConnection();
        $condition =
            "{$this->getTable('rueducommerce_profile_products')}.product_id IN (" . implode(',', $productIds) . ")";
        $dbh->delete($this->getTable('rueducommerce_profile_products'), $condition);
        return $this;
    }
    ...
    public function addProducts($productIds, $profileId)
    {
        ...
        $dbh = $this->getConnection();
        $dbh->insertMultiple($this->getTable('rueducommerce_profile_products'), $data);
        return $this;
    }
    ...
    {
        if ($productId > 0) {
            $profileTable = $this->getTable('rueducommerce_profile_products');
            $productProfile = \Magento\Framework\App\ObjectManager::getInstance()
                ->get('Ced\RueDuCommerce\Model\ProfileProduct')->loadByField('profile_id', $profileId);
