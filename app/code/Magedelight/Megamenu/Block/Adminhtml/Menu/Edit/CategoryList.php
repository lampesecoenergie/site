...
class CategoryList extends \Magento\Catalog\Block\Adminhtml\Category\Tree
{
     ...
     * @return array
     */
    public function getRootCategoryFromStore()
    {
        $menublock = $this->getParentBlock();
        $menu = $menublock->getMenuFromMenuId();
    ...
    public function mapStoreIds()
    {
        ...
        foreach ($stores as $store) {
            $this->allStoreIds[$store->getStoreId()] = $store->getRootCategoryId();
        }
    }
    ...
    {
        $rootCategories = $this->getRootCategoryFromStore();
        if (in_array(0, $rootCategories)) {
            /* all storeview selected */
            if (count(array_unique($this->allStoreIds)) === 1) {
                return end($this->allStoreIds);
            }
        } else {
