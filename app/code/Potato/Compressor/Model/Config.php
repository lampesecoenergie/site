...
class Config
{
    ...
    const IMAGE_PRODUCT_GALLERY = 'po_compressor/image/speed_up_product_gallery';
    ...
    const ADVANCED_IMAGES_FROM_LAZY_LOAD = 'po_compressor/advanced/exclude_images_from_lazy_load';
     ...
     * @return bool
     */
    public function isImageProductGallerySpeedUp($store = null)
    {
        if (null === $store) {
            $store = $this->storeManager->getStore()->getId();
        }
        return (bool)$this->scopeConfig->getValue(
                self::IMAGE_PRODUCT_GALLERY,
                ScopeInterface::SCOPE_STORE,
                $store
            ) && $this->isImageEnabled($store);
    }
     ...
     * @return array
     */
    public function getExcludeImagesFromLazyLoad($store = null)
    {
        if (null === $store) {
            $store = $this->storeManager->getStore()->getId();
        }
        $rawText = trim($this->scopeConfig->getValue(
            self::ADVANCED_IMAGES_FROM_LAZY_LOAD,
            ScopeInterface::SCOPE_STORE,
            $store
        ));
        if (strlen($rawText) === 0) {
     ...
     * @return bool
     */
    public function isRequestAvailableForModule($request, $store = null)
    {
        $originalPathInfo = $request->getOriginalPathInfo();
        foreach ($this->getExcludeRouters($store) as $route) {
