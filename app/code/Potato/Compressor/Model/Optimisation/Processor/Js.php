    ...
    public function merge(&$html)
    {
        ...
        for ($i = 1; $i < count($tagList); $i++) {
            ...
            if (array_key_exists('src', $previousAttributes)) {
                if (!$this->fileHelper->isInternalUrl($previousAttributes['src'])
                    || !file_exists($this->fileHelper->getLocalPathFromUrl($previousAttributes['src']))) {
                    $isNeedNewGroup = true;
                }
            }
            $currentAttributes = $currentTag->getAttributes();
            if (array_key_exists('src', $currentAttributes)) {
                if (!$this->fileHelper->isInternalUrl($currentAttributes['src'])
                    || !file_exists($this->fileHelper->getLocalPathFromUrl($currentAttributes['src']))) {
                    $isNeedNewGroup = true;
                }
            }
            if ($isNeedNewGroup) {
