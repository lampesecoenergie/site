...
class Processor
{
    ...
    public function processHtmlResponse($response)
    {
        ...
        if ($this->config->isImageProductGallerySpeedUp()) {
            Profiler::start('po-compressor-image-product-gallery');
            $this->imageProcessor->processFixProductGallery($resultHtml);
            Profiler::stop('po-compressor-image-product-gallery');
        }
